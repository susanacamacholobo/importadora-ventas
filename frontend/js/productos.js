...
export async function cargarProductos(API_URL) {
  ...
  try {
    ...
    allProducts = data.map(p => ({
      ...
      badge:     p.stock_total === 0 ? 'agotado' : p.stock_total <= 5 ? 'low' : ''
    }));

    document.getElementById('total-prods').textContent   = allProducts.length;
    document.getElementById('stock-critico').textContent = allProducts.filter(p => p.stock <= 5).length;

    renderProducts(allProducts);
    llenarFiltroProveedores();
  } catch(err) {
    grid.innerHTML = '<p style="color:var(--red);font-size:13px;grid-column:1/-1">Error conectando con la API.</p>';
    console.error(err);
  }
}
